		...
		>
			...
			{mapError && (
				<div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-dotori-100 px-5 text-center dark:bg-dotori-800">
						<p className="text-body text-dotori-500 dark:text-dotori-200">{mapErrorTitle}</p>
						{developerHint && (
							<p className="text-body-sm text-dotori-500 dark:text-dotori-300">{developerHint}</p>
						)}
						{isProd && (
							<button
								type="button"
								onClick={handleRetry}
								disabled={!hasApiKey}
								className="mt-1 min-h-11 rounded-xl bg-dotori-500 px-4 py-2 text-body font-semibold text-white disabled:bg-dotori-200 disabled:text-dotori-500 dark:disabled:bg-dotori-800 dark:disabled:text-dotori-300"
							>
								재시도
							</button>
						)}
			...
			{!isLoading && !mapError && facilities.length >= 5 && (
				<div className="absolute inset-x-2 bottom-2 flex justify-center">
						<p className="rounded-lg bg-white/90 px-2.5 py-1 text-body-sm text-dotori-600 shadow-sm dark:bg-dotori-950/80 dark:text-dotori-200 dark:shadow-none">
