...
const { authMock, dbConnectMock, addInterestMock, removeInterestMock, standardLimiterCheckMock, userFindByIdMock } =
  vi.hoisted(() => ({
    authMock: vi.fn(),
    dbConnectMock: vi.fn(),
...
// Use the real NotFoundError so withApiHandler recognizes it via instanceof
...
describe('POST /api/users/me/interests', () => {
  it('returns 404 with compatible error payload when facility does not exist', async () => {
    addInterestMock.mockRejectedValueOnce(
      new NotFoundError('존재하지 않는 시설입니다'),
    )
