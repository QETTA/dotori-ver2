...
export interface IFacility extends Document {
	name: string;
	type: "국공립" | "민간" | "가정" | "직장" | "협동" | "사회복지" | "국립유치원" | "공립유치원" | "사립유치원";
...
const FacilitySchema = new Schema<IFacility>(
	{
		...
		type: {
			type: String,
			enum: ["국공립", "민간", "가정", "직장", "협동", "사회복지", "국립유치원", "공립유치원", "사립유치원"],
			required: true,
		},
		facilityCategory: {
			type: String,
			enum: ["daycare", "kindergarten"],
		},
		status: {
			type: String,
