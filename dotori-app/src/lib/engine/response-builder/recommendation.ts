...
export async function buildRecommendationResponse(
	...
	try {
		...
		if (facilities.length === 0) {
			...
			const content =
				aiResponse.success && aiResponse.content
					? aiResponse.content
					: "조건에 맞는 시설을 찾지 못했어요. 검색 조건을 넓혀볼까요?";
			return {
				content,
				blocks: [
					{ type: "text", content },
		...
		const fallbackContent = `${regionLabel} 시설 ${facilities.length}곳을 찾았어요!`;
		const content =
			aiResponse.success && aiResponse.content
				? aiResponse.content
				: fallbackContent;
...
export async function buildComparisonResponse(
	...
	try {
		...
		if (facilities.length < 2) {
			return fallbackResponse("비교할 시설이 충분하지 않아요.");
		}
