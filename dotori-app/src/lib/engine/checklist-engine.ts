...
interface ChecklistInput {
	facilityType: "국공립" | "민간" | "가정" | "직장" | "협동" | "사회복지" | "국립유치원" | "공립유치원" | "사립유치원";
...
const TYPE_SPECIFIC_DOCS: Record<string, ChecklistItem[]> = {
	...
	국립유치원: [
		{
			id: "kinder-national-1",
			text: "입학원서",
			detail: "해당 유치원에서 양식 배부 (추첨 선발)",
			checked: false,
			required: true,
		},
		{
			id: "kinder-national-2",
			text: "생활기록부 (전입 시)",
			detail: "이전 유치원에서 발급",
			checked: false,
			required: false,
...
export function generateChecklist(input: ChecklistInput): ChecklistBlock {
	...
	// 2. 유형별 추가 서류
	const isKindergarten = input.facilityType.endsWith("유치원");
	const typeDocs = TYPE_SPECIFIC_DOCS[input.facilityType];
	if (typeDocs && typeDocs.length > 0) {
		categories.push({
			title: isKindergarten
				? `${input.facilityType} 추가 서류`
				: `${input.facilityType} 추가 서류`,
	...
	return {
		type: "checklist",
		title: isKindergarten
			? `${input.facilityType} 입학 서류 체크리스트`
			: `${input.facilityType} 입소 서류 체크리스트`,
