...
describe("classifyIntent", () => {
	...
	it("classifies '유치원 찾아줘' as recommend", () => {
		expect(classifyIntent("유치원 찾아줘")).toBe("recommend");
	});

	it("classifies '유치원 이동' as transfer", () => {
		expect(classifyIntent("다른 유치원으로 이동하고 싶어요")).toBe("transfer");
	});

	it("classifies '유보통합 뭐야' as knowledge", () => {
		expect(classifyIntent("유보통합이 뭐야?")).toBe("knowledge");
	});
