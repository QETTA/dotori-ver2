...
import { FireIcon, HeartIcon as HeartSolidIcon } from '@heroicons/react/24/solid'
import { motion } from 'motion/react'
import { useSession } from 'next-auth/react'
import Link from 'next/link'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { categoryLabel, tabs, tabToCategoryParam, type TabLabel } from './_lib/community-constants'
import type {
...
const CATEGORY_BADGE_COLOR: Record<CommunityPostWithViews['category'], 'forest'> = {
  feedback: 'forest',
  review: 'forest',
  question: 'forest',
  info: 'forest',
    ...
    <div className={COMMUNITY_PAGE_CLASS}>
      ...
      >
        <div className="relative flex min-h-11 items-center justify-between gap-2 pb-2">
          ...
          <img
            ...
            alt=""
            aria-hidden="true"
            className="pointer-events-none absolute top-1/2 right-0 h-6 w-6 -translate-y-1/2 opacity-[0.2]"
          />
        </div>
              ...
              return (
                <motion.div key={tab} {...tap.chip}>
                  <DsButton
                    ...
                    className={cn(
                      'min-h-11 min-w-max rounded-full px-3.5 font-medium transition-transform active:scale-100',
                      'text-label',
                      isActive
                        ? cn(
                            DS_STATUS.full.pill,
                            'text-white shadow-sm shadow-dotori-200 dark:bg-dotori-500 dark:shadow-none',
                          )
      ...
      <div className="space-y-4 px-4 pt-4">
        <motion.section {...fadeUp} className="mb-1">
          <div className={cn(COMMUNITY_SECTION_CLASS, DS_GLASS.CARD)}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={BRAND.watermark}
              alt=""
              aria-hidden="true"
              className={cn(COMMUNITY_WATERMARK_CLASS, 'top-0 left-0')}
            />
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 border-t border-dotori-100/80 pt-3">
                {communityStats.map((stat) => (
                  <div
                    key={stat.label}
                    className={cn(
                      'rounded-xl border border-dotori-100/70 bg-white/90 px-2 py-2 text-center dark:border-dotori-800/70 dark:bg-dotori-950/80',
                      DS_GLASS.CARD,
                    )}
                  >
        ...
        {isTransitionMonth ? (
          <motion.div
            {...fadeUp}
            className={cn(
              'mb-4 rounded-3xl p-4 text-forest-700 shadow-sm ring-1 shadow-dotori-100/60 ring-dotori-100/70 dark:text-forest-200 dark:shadow-none dark:ring-dotori-700/50',
              WARM_SURFACE,
              DS_GLASS.CARD,
              'text-body-sm',
            )}
          ...
          <motion.div {...fadeIn}>
            <motion.div {...tap.button} className="mb-4">
              <DsButton
                ...
                className={cn(
                  'flex min-h-11 w-full items-center gap-2.5 rounded-3xl p-3.5 text-left shadow-sm ring-1 shadow-dotori-100/60 ring-dotori-100/70 dark:shadow-none dark:ring-dotori-700/50',
                  MOTION_SMOOTH,
                  WARM_SURFACE,
                  DS_GLASS.CARD,
        ...
        {isLoading ? (
          ...
          <div>
            <motion.ul {...stagger.container} className="space-y-2.5">
              {posts.map((post) => {
                ...
                const postCategoryBadgeColor = CATEGORY_BADGE_COLOR[post.category]
                const postTitle =
                  post.title?.trim() ||
                ...
                return (
                  <motion.li
                    ...
                    {...tap.card}
                    className={cn(
                      COMMUNITY_LIST_ITEM_CLASS,
                      isPopularPost
                        ? COMMUNITY_LIST_ITEM_HOT_CLASS
                        : 'bg-gradient-to-b from-dotori-50 via-dotori-50/60 to-white dark:from-dotori-900 dark:to-dotori-950 dark:shadow-none dark:ring-dotori-800',
                    )}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      ...
                      aria-hidden="true"
                      className={cn(
                        COMMUNITY_WATERMARK_CLASS,
                        'top-1/2 left-1/2 h-28 w-28 -translate-x-2/3 -translate-y-1/2',
                      )}
                    />
                    ...
                    <Link
                      ...
                      className="mt-3 block border-b border-dotori-100/70 pb-3"
                    >
                      <h3
                        className={cn(
                          DS_TYPOGRAPHY.h3,
            ...
            {isLoadingMore ? (
              <div className="mt-4 flex flex-col items-center py-3">
                ...
                <p className={cn('mt-2 text-body-sm text-dotori-600 dark:text-dotori-300')}>
                  다음 글을 불러오는 중...
                </p>
              </div>
            ) : null}
          ...
          <EmptyState
            title="이웃 글이 아직 없어요"
            variant="default"
            description={tabEmptyMessages[activeTab]}
            actionLabel="첫 글 쓰기"
      ...
      {!isLoading && !error && posts.length > 0 ? (
        ...
        >
          <Link
            ...
            aria-label="글쓰기"
            className={cn(
              'grid min-h-14 min-w-14 place-items-center rounded-full bg-gradient-to-br from-dotori-900 via-dotori-800 to-dotori-700 shadow-lg ring-2 shadow-dotori-900/20 ring-white/80 dark:from-dotori-500 dark:via-dotori-500 dark:to-dotori-400 dark:shadow-none dark:ring-dotori-900/60',
              DS_GLASS.FLOAT,
            )}
          >
