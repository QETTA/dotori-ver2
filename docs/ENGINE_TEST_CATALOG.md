...
- 위치: `src/lib/engines/to-prediction/` (구현 예정)
- 설명: 연령반별 잔여TO 계산 + AI 입소 확률 예측
- 입력: 시설 정원/현원, 행정동 인구, 계절 보정값
- 출력: 예측 TO 수, 입소 확률 (0~1)

| Case ID | Priority | Scenario | Input Variants | Expected | Notes |
|---------|----------|----------|----------------|----------|-------|
| ENG-TO-OK-001 | P0 | 기본 TO 계산 (정원 - 현원) | 정원20, 현원15 | TO=5 | 순수 계산 |
| ENG-TO-OK-002 | P1 | 졸업 보정 TO | 정원20, 현원20, 졸업예정3 | TO=3 | 연말 시나리오 |
| ENG-TO-BND-001 | P1 | 정원 = 현원 (만실) | 정원20, 현원20 | TO=0 | 경계값 |
| ENG-TO-BND-002 | P1 | 현원 > 정원 (초과) | 정원20, 현원22 | TO=0, 경고 | 데이터 이상 |
| ENG-TO-ERR-001 | P0 | 음수/null 입력 | 정원=-1 | VALIDATION_ERROR | 입력 검증 |
| ENG-TO-VAL-001 | P0 | 시설매력도 범위 | 매력도=1.5 (범위 초과) | VALIDATION_ERROR | 0~1 범위 |
| ENG-TO-REG-001 | P0 | 인구 데이터 누락 시 fallback | 행정동 인구=null | 기본 수요 적용 | 그레이스풀 |

### 8.2 전자서명 시스템 테스트

- 위치: `src/lib/esign/` (구현 예정)
- 설명: 7종 서류 Canvas 서명 + PDF 생성 + 감사추적
- 입력: 서명 이미지(base64), 서류 템플릿 ID, 사용자 정보
- 출력: 서명된 PDF, 감사추적 로그
- 부작용: DB 저장 (서명 기록), 모두싸인 API 호출 (법적 문서)

| Case ID | Priority | Scenario | Input Variants | Expected | Notes |
|---------|----------|----------|----------------|----------|-------|
| ENG-SIGN-OK-001 | P0 | 자체 서명 (Canvas) | 유효 base64 + 입소동의서 | PDF 생성 + 감사로그 | 일반 동의서 |
| ENG-SIGN-OK-002 | P1 | 모두싸인 서명 | 법적 문서 ID | 모두싸인 API 호출 | mock 필수 |
| ENG-SIGN-OK-003 | P1 | 7종 일괄 서명 | 7개 템플릿 | 7개 PDF + 감사추적 | 퍼널 완결 |
| ENG-SIGN-ERR-001 | P0 | 빈 서명 이미지 | base64="" | VALIDATION_ERROR | 서명 필수 |
| ENG-SIGN-ERR-002 | P0 | 모두싸인 API 실패 | timeout | fallback 안내 | 재시도 로직 |
| ENG-SIGN-ST-001 | P1 | 서명 상태 전이 | 대기→서명→완료 | 상태 정확 | 멱등성 확인 |

### 8.3 B2B API 계약 테스트

- 위치: `src/app/api/partner/` (구현 예정)
- 설명: B2B SaaS 파트너 API (시설 대시보드, TO 통계, CPA 과금)
- 입력: 파트너 API 키, 시설 ID, 조회 기간
- 출력: 시설 통계 JSON, CPA 이벤트 로그
- 부작용: CPA 과금 기록, 사용량 추적

| Case ID | Priority | Scenario | Input Variants | Expected | Notes |
|---------|----------|----------|----------------|----------|-------|
| ENG-B2B-OK-001 | P0 | 시설 통계 조회 | 유효 API 키 + 시설ID | 통계 JSON | 인증 필수 |
| ENG-B2B-OK-002 | P1 | CPA 이벤트 기록 | 견학 신청 이벤트 | 과금 로그 저장 | 멱등 |
| ENG-B2B-ERR-001 | P0 | 잘못된 API 키 | 무효 키 | AUTH_ERROR | 401 |
| ENG-B2B-ERR-002 | P0 | 구독 티어 초과 | Basic 티어 + 프로 기능 | TIER_LIMIT_ERROR | 403 |
| ENG-B2B-VAL-001 | P0 | 잘못된 시설 ID | 존재하지 않는 ID | NOT_FOUND | 404 |
| ENG-B2B-CONC-001 | P2 | 동시 CPA 기록 | 동일 견학 2건 | 중복 방지 | 멱등성 |
